...
- Кнопка **«Действия»** — забиндить **любую** команду Revit, которой нет на
  ленте: контекстное меню (скрыть элемент/категорию, переопределить графику),
  панель вида (показать скрытые), диспетчер проекта и т.д. Полный список
  команд читается из `KeyboardShortcuts.xml` Revit (локализованные имена +
  пути меню); если файла ещё нет — по ссылке открывается окно «Сочетания
  клавиш», Revit его создаёт. Команда выполняется над выделенным объектом
